...
export class LoginService {
  ...
  login(email: string, password: string): boolean {
    ...
    if ((emailFixed == user1 || emailFixed == user2 || emailFixed == user3) && (emailFixed == password)) {
      localStorage.setItem(tokenName, emailFixed);
      localStorage.setItem(tokenInfoName, emailFixed);
      return true;
    } else {
      this.logout();
      return false;
